(function () {
  async function fetchHtml(url) {
    ...
    if (customProxy) {
      ...
      console.debug(`[manhwabuddy] Added custom proxy: ${formattedProxy}`);
    }
    ...
    try {
      ...
      throw new Error(
        `[manhwabuddy] Failed to fetch after attempts:\n${attempts.join("\n")}`,
      );
    }
  }
  ...
  async function primaryGetAvailable(url) {
    ...
    try {
      ...
      if (/Caution to under-aged viewers|Are you over 18/i.test(bodyText)) {
        console.warn(
          "[manhwabuddy] 18+ interstitial rilevato: la chapter-list potrebbe non essere presente",
        );
      }

      // Struttura specifica: div.box ul.chapter-list > li.myfut > a[title]
      let anchors = Array.from(
        doc.querySelectorAll("div.box ul.chapter-list li.myfut a[title]"),
      );
      console.debug(
        "Selettore div.box ul.chapter-list li.myfut a[title] ->",
        anchors.length,
      );
      if (anchors.length === 0) {
        // Estendi ricerca all'intera chapter-list
        anchors = Array.from(
          doc.querySelectorAll("ul.chapter-list li a[title]"),
        );
        console.debug(
          "Fallback ul.chapter-list li a[title] ->",
          anchors.length,
        );
      }
      if (anchors.length === 0) {
        ...
        anchors = Array.from(
          doc.querySelectorAll('ul[class*="chapter-list" i] li a[title]'),
        );
        console.debug(
          'Fallback ul[class*="chapter-list" i] li a[title] ->',
          anchors.length,
        );
      }
      if (anchors.length === 0) {
        ...
        anchors = Array.from(
          doc.querySelectorAll('ul.chapter-list li[class*="my" i] a[title]'),
        );
        console.debug(
          'Fallback ul.chapter-list li[class*="my" i] a[title] ->',
          anchors.length,
        );
      }
      ...
      if (candidates.length === 0) {
        ...
        const altAnchors = Array.from(
          doc.querySelectorAll("ul.chapter-list a"),
        );
  ...
  async function alternativeGetAvailable(url) {
    ...
    try {
      ...
      if (/Caution to under-aged viewers|Are you over 18/i.test(bodyText)) {
        console.warn(
          "[manhwabuddy] 18+ interstitial rilevato (alternative): la pagina potrebbe essere bloccata",
        );
      }
      ...
      for (const sel of selectors) {
        ...
        for (const a of anchors) {
          const title = a.getAttribute("title") || "";
          const n =
            extractChapterNumberFromTitle(title) ??
            extractChapterNumberFromText(a.textContent || "") ??
            extractChapterNumberFromHref(a.getAttribute("href") || "");
  ...
  async function getAvailableChapters(url) {
    try {
      ...
      console.warn(
        `Primary scraping manhwabuddy fallito per ${url}:`,
        err.message,
      );
  ...
  const provider = {
    name: "manhwabuddy",
    match: (url, host) =>
      (host || "").toLowerCase().includes("manhwabuddy") ||
      (url || "").toLowerCase().includes("manhwabuddy"),
