class MangaManager {
  ...
  async loadFromS3() {
    try {
      ...
      try {
        ...
        if (validRowCount > 0) {
          // Costruisci la query di selezione in base alla presenza della colonna status
          const statusSelect = hasStatus
      ...
      if (error.code === "NoSuchKey") {
        console.log(
          "File Parquet non trovato su S3, inizializzo con tabella vuota",
        );
      } else {
        console.error("Errore nel caricamento da S3:", error);
        // Previeni il loop infinito
        if (
          !this.isHandlingSchemaConflict &&
          ...
          console.log(
            "Ricreando la tabella per risolvere conflitti di schema...",
          );
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Verifica presenza colonna status anche qui se possibile, o usa default
          // Per semplicità qui assumiamo che se c'è conflitto stiamo provando a recuperare il possibile
          ...
          try {
            ...
            console.log(
              "Impossibile verificare schema parquet in recovery:",
              e,
            );
          }
          ...
          console.log(
            "Dati ricaricati con successo dopo la ricreazione della tabella",
          );
        } catch (loadError) {
          console.log(
            "Impossibile ricaricare i dati dal Parquet, continuando con tabella vuota:",
            loadError.message,
          );
        }
      } finally {
  ...
  async addManga(
    ...
    const conn = await this.db.connect();
    try {
      // Sincronizza started e status
      if (started && status === "unread") status = "reading";
  ...
  async deleteManga(link) {
    ...
    // Ricompatta gli ID dopo l'eliminazione
    await this.recompactIds();

    // Salva automaticamente su S3 dopo l'eliminazione e ricompattazione
    await this.saveToS3();
  }
  ...
  async updateManga(
    ...
    try {
      ...
      if (status === null) {
        // If status not provided, derive from started
        status = started ? "reading" : "unread";
      } else {
        // If status provided, sync started
        started = status === "reading";
      }
  ...
  async getStats() {
    ...
    try {
      ...
      return {
        total: row.total,
        started: row.reading, // Keep for legacy
        not_started: row.unread, // Keep for legacy
        reading: row.reading,
        unread: row.unread,
  ...
  async recompactIds() {
    ...
    try {
      ...
      const maxIdResult = await conn.query(
        "SELECT MAX(id) as max_id FROM manga",
      );
